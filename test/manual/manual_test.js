    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status === 200) {
          ...
          reject(
            new Error(
              `Couldn't fetch token from ${this.authEndpoint}; got ${
                xhr.status
              } ${xhr.responseText}.`,
            ),
          );
      ...
      xhr.ontimeout = () => {
        reject(
          new Error(
            `Request timed out while fetching token from ${this.authEndpoint}`,
          ),
        );
      };
...
let subscription1 = instance.subscribeResuming(
  createSubscribeOptions('my-feed'),
);
