...
export class BaseSubscription {
    ...
    public getHeaders(): ElementsHeaders {
        return responseToHeadersObject(this.transport.getAllResponseHeaders());
    }
    ...
    private onLoading(): void {
        ...
        if(this.transport.status === 200){
            ...
            if(this.state === BaseSubscriptionState.OPENING) {
                ...
                this.onOpen(responseToHeadersObject(this.transport.getAllResponseHeaders()));
            }
    ...
    private onDone(): void {
        if (this.transport.status === 200) {
            ...
            //Something terrible has happened. Most likely a network error. XHR is useless at that point.
                ...
                this.onError(ErrorResponse.fromXHR(this.transport.getXhr()));
            }
        }
    }
    ...
    private onChunk(): Error {
        this.assertStateIsIn(BaseSubscriptionState.OPEN);
        let response = this.transport.responseText;
