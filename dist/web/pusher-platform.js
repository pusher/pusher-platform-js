...
var WebSocketTransport = (function () {
    ...
    WebSocketTransport.prototype.connect = function () {
        ...
        this.socket.onopen = function (event) {
            ...
            _this.pingInterval = window.setInterval(function () {
                ...
                _this.pongTimeout = window.setTimeout(function () {
                    ...
                    window.console.log("Calling close because pong response timeout");
        ...
        this.socket.onerror = function (event) {
            window.console.log("Received an error in onerror");
            window.console.log(event);
            _this.close(new network_1.NetworkError('Connection was lost.'));
        };
        this.socket.onclose = function (event) {
            if (!_this.forcedClose) {
                window.console.log("Not forced close in onclose so we will go to tryReconnectIfNeeded");
                _this.tryReconnectIfNeeded();
                return;
            }
            window.console.log("Is there a closedError? " + _this.closedError);
    ...
    WebSocketTransport.prototype.close = function (error) {
        ...
        window.console.log("Doing a forced close");
        ...
        window.console.log("THIS.SOCKET.CLOSE ABOUT TO BE CALLED");
        this.socket.close();
    ...
    WebSocketTransport.prototype.receiveMessage = function (event) {
        ...
        try {
            ...
            window.console.log("Calling close because invalid JSON in message");
        ...
        if (nonValidMessageError) {
            window.console.log("Calling close because message is invalid");
    ...
    WebSocketTransport.prototype.onCloseMessage = function (closeMessage) {
        ...
        if (typeof statusCode !== 'number') {
            window.console.log("Calling close because of invalid EOS Status Code");
            return this.close(new Error('Close message: Invalid EOS Status Code'));
        }
        if (typeof headers !== 'object' || Array.isArray(headers)) {
            window.console.log("Calling close because of invalid EOS ElementsHeaders");
            return this.close(new Error('Close message: Invalid EOS ElementsHeaders'));
        }
        window.console.log("Calling close because at end of onCloseMessage function");
        var errorInfo = {
    ...
    WebSocketTransport.prototype.onPongMessage = function (message) {
        ...
        if (this.lastSentPingID !== receviedPongID) {
            window.console.warn("Received pong with ID " + receviedPongID + " but lastSentPingID was " + this.lastSentPingID);
        }
        window.console.log("Received pong ID " + receviedPongID);
        window.clearTimeout(this.pongTimeout);
        delete this.pongTimeout;
        this.lastSentPingID = null;
    };
    WebSocketTransport.prototype.onPingMessage = function (message) {
        ...
        window.console.log("Received ping ID " + receviedPingID);
